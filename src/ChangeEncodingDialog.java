...
public class ChangeEncodingDialog extends BaseDialog implements ActionListener, ListSelectionListener {
  ...
  private void toChangeEncoding(File file, String strText, LineSeparator lineSeparator) {
    ...
    try {
      if (this.chkRetainSource.isSelected()) {
        String fileName = this.getFileNameWithEncoding(file.getName(), this.charEncoding);
   ...
   * @return 转换后的新文件名
   */
  private String getFileNameWithEncoding(String fileName, CharEncoding charEncoding) {
    if (Util.isTextEmpty(fileName) || charEncoding == null) {
      return "";
    }
