...
public final class Util {
  ...
  public static final String VERSION = "V5.3"; // 软件版本号
  ...
  public static final String[] SHORTCUT_NAMES = new String[] {
      ...
      "关闭左侧","关闭右侧","关闭全部","冻结文件","打印","保存设置","清空最近编辑列表","退出","撤销","重做","剪切","复制","粘贴",
      "全选","删除","拆分文件","切换为大写","切换为小写","大小写互换","复制当前文件名到剪贴板","复制当前文件路径到剪贴板","复制当前目录路径到剪贴板","复制所有文本到剪贴板","复写当前行","删除当前行","删除重复行","删除空行",
      ...
      "后一个文档","定位到行首","定位到行尾","定位到文件首","定位到文件尾","统计信息","窗口管理","计算器","散列/哈希加密","加密/解密","URL编码/解码","切割文件","拼接文件","转换文件编码","转换文件换行符","文本格式转换","数字进制转换","时间戳转换","单位换算","三角函数","题库","统计求和","统计平均值","统计最大值","统计最小值","统计升序排列","统计降序排列","精简gradle依赖","精简Flutter依赖","帮助主题","关于"
      }; // 快捷键的名称
   ...
   */
  public static String encodeTextByBase64(String text) {
    try {
      ...
      for (int i = 0; i < length; i += 3) {
        int chunk = 0;
        int bytesInChunk = Math.min(3, length - i);
        for (int j = 0; j < bytesInChunk; j++) {
          chunk = (chunk << 8) | (inputBytes[i + j] & 0xFF);
        }
        if (bytesInChunk < 3) {
          padding = 3 - bytesInChunk;
          chunk <<= 8 * padding;
        }
        for (int j = 0; j < 4; j++) {
          if (j < 4 - padding) {
            int index = (chunk >> (6 * (3 - j))) & 0x3F;
            encoded.append(BASE64_ALPHABET.charAt(index));
          } else {
            encoded.append('=');
          }
        }
   ...
   */
  public static String decodeTextByBase64(String text) {
    ...
    for (int i = 0; i < length; i++) {
      ...
      int index = BASE64_ALPHABET.indexOf(c);
      chunk = (chunk << 6) | index;
      bits += 6;
      if (bits >= 8) {
        bits -= 8;
        output[outputIndex++] = (byte) ((chunk >> bits) & 0xFF);
      }
    }
    try {
      return new String(output, 0, outputIndex, "UTF-8");
    } catch (Exception x) {
      x.printStackTrace();
    }
    return text;
