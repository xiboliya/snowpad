...
3关闭文件#关闭文件，包括：关闭当前、其他、左侧、右侧、全部。
4关闭当前#关闭当前编辑的文件。
4关闭其它#关闭除了当前编辑的所有其它文件。
4关闭左侧#关闭当前编辑文件的所有左侧文件。
4关闭右侧#关闭当前编辑文件的所有右侧文件。
4关闭全部#关闭编辑区的所有文件。
3冻结文件#冻结当前文件，亦即禁止修改当前文件。
3打印#打印当前文件。
3保存设置#保存当前的设置和已打开的文件等信息。
...
4大小写互换#将当前文本域中的选择文本的大写切换为小写或小写切换为大写。
...
3快速向下查找#在当前文本域中快速向后查找当前选择的字符串，而并不改变默认查找的字符串。
3快速向上查找#在当前文本域中快速向前查找当前选择的字符串，而并不改变默认查找的字符串。
3替换#将当前文本域中某字符串替换为指定字符串。
3标记#在当前文本域中查找指定字符串并突出显示，包括：高亮显示、为所在行添加书签。
3转到#转到指定的行号、偏移量或百分比。
...
4GB18030格式#设置当前文件的编码格式为GB18030格式，此为默认格式。GB18030格式是继GB2312与GBK之后的支持汉字最多的新编码格式。
...
3文件视图#设置当前文件的查看视图。共有2种格式：文本视图、二进制视图。
4文本视图#普通文本文件的视图方式，即在文本域中显示文件的文本内容。此为默认方式。
4二进制视图#二进制的视图方式，即将文件的每个字节以十六进制形式显示在文本域中，每行16个字节，各字节间以空格分隔。
...
3颜色设置#设置文本域中各状态下字体、背景和光标的颜色，包括：全部反色、全部补色等。
4全部反色#将文本域中各状态下字体、背景和光标的颜色进行反色设置。
4全部补色#将文本域中各状态下字体、背景和光标的颜色进行补色设置。
3配色方案#设置预置的配色方案，包括：字体颜色、背景颜色、光标颜色、选区字体颜色、选区背景颜色、匹配括号背景颜色、当前行背景颜色、匹配文本背景颜色、标记文本背景颜色、书签颜色、行号栏字体颜色、行号栏背景颜色、高亮显示颜色等。
4配色方案1#设置第1种预置的配色方案。
4配色方案2#设置第2种预置的配色方案。
4配色方案3#设置第3种预置的配色方案。
4配色方案4#设置第4种预置的配色方案。
...
3光标定位#在文本域中快速定位光标位置。
4定位到行首#将光标快速定位到当前行的开头。
4定位到行尾#将光标快速定位到当前行的结尾。
4定位到文件首#将光标快速定位到当前文件的开头。
4定位到文件尾#将光标快速定位到当前文件的结尾。
3统计信息#显示当前的文件信息与当前编辑的文本域信息。
3窗口管理#管理当前编辑的所有文档，包括：激活、保存、关闭、排序、上移、下移等功能。
2工具#此菜单中包含一些工具类功能。
3计算器#简单的计算器，主要功能包括：整数和小数的＋－×÷四则运算，百分比%运算，圆周率π运算等。
3散列/哈希加密#计算指定文本或文件的散列/哈希加密值，包括：MD5、SHA、SHA-224、SHA-256、SHA-384、SHA-512等散列/哈希加密算法。
3加密/解密#计算指定文本的加密值和解密值，包括：Base64等加密算法。
3URL编码/解码#对Url进行编码和解码。
...
4最大值#计算选中的多行数字的最大值。
4最小值#计算选中的多行数字的最小值。
4升序排列#将选中的多行数字进行升序排列。
4降序排列#将选中的多行数字进行降序排列。
3专业工具#特定的行业领域的功能。
4精简Gradle依赖#将在Android开发中的Gradle依赖树精简为单纯的依赖版本列表。
4精简Flutter依赖#将在Flutter开发中的依赖树精简为单纯的依赖版本列表。
